...
function(apigen_android_archive)
  ...
  cmake_parse_arguments(apigen_android_archive "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})
  ...
  if(NOT ${GENERATOR} MATCHES "android")
    message(
      FATAL_ERROR
        "apigen_android_archive() depends on apigen_android_archiver() configured with generator 'android'"
    )
  endif()

  set(APIGEN_ANDROID_ARCHIVE_OUTPUT_DIR
      ${CMAKE_CURRENT_BINARY_DIR}/apigen/${GENERATOR}-android-archive)
  set(APIGEN_ANDROID_ARCHIVE_JNI_OUTPUT_DIR
      ${APIGEN_ANDROID_ARCHIVE_OUTPUT_DIR}/jni/${CMAKE_ANDROID_ARCH_ABI})
  ...
  add_custom_command(
    TARGET ${apigen_android_archive_TARGET}
    POST_BUILD
    COMMAND ${CMAKE_COMMAND} ARGS -E make_directory ${APIGEN_ANDROID_ARCHIVE_JNI_OUTPUT_DIR}
    COMMAND ${CMAKE_COMMAND} ARGS -E copy $<TARGET_FILE:${apigen_android_archive_TARGET}>
            ${APIGEN_ANDROID_ARCHIVE_JNI_OUTPUT_DIR}
    COMMAND ${CMAKE_COMMAND} ARGS -E copy ${APIGEN_ANDROID_ARCHIVE_MANIFEST_XML}
            ${APIGEN_ANDROID_ARCHIVE_OUTPUT_DIR}/
  ...
  foreach(asset ${apigen_android_archive_ASSETS})
    # NOTE: Resources are symlinked, but some links will be invalid because the targets that build
    # the assets haven't been built. Use tar to transfer the links that are valid to the build
    # directory and preserve the directory hierarchy.
  ...
  set_target_properties(
    ${apigen_android_archive_TARGET} PROPERTIES APIGEN_ANDROID_ARCHIVE_JNI_OUTPUT_DIR
                                                ${APIGEN_ANDROID_ARCHIVE_JNI_OUTPUT_DIR})
  ...
  if(apigen_android_archive_ADD_JAR)
    ...
    # cmake-format: off
    add_custom_command(
      TARGET ${apigen_android_archive_TARGET} POST_BUILD
      COMMAND ${Java_JAR_EXECUTABLE} -cfM ${APIGEN_ANDROID_ARCHIVE_CLASSES_JAR}
        -C ${JAR_MERGE_DIR} .)
    # cmake-format: on
  else()
    ...
    add_custom_command(
      TARGET ${apigen_android_archive_TARGET} POST_BUILD
      COMMAND ${CMAKE_COMMAND} ARGS -E copy ${APIGEN_JAVA_JAR}
              ${APIGEN_ANDROID_ARCHIVE_CLASSES_JAR})
  endif()
  ...
  install(FILES ${APIGEN_ANDROID_ARCHIVE} DESTINATION lib)
