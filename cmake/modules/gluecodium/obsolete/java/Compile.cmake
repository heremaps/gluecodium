...
function(collect_java_dependencies TARGET JAVA_DEPENDENCIES JAVA_DEPENDENCIES_DIRS)
  ...
  # CMake doesn't want us to ask INTERFACE_LIBRARY targets for arbitrary properties, instead of just
  # returning it's not defined it fails because it's not whitelisted for INTERFACE_LIBRARY. So we
  # have to do an additional check before the real check.
...
function(apigen_java_compile)
  ...
  set(multiValueArgs LOCAL_DEPENDENCIES LOCAL_DEPENDENCIES_DIRS LOCAL_JARS REMOTE_DEPENDENCIES
                     LOCAL_SOURCES_DIRS)
  cmake_parse_arguments(apigen_java_compile "${options}" "${oneValueArgs}" "${multiValueArgs}"
                        ${ARGN})
  ...
  if(NOT ${GENERATOR} MATCHES "android")
    message(
      FATAL_ERROR
        "apigen_java_compile() depends on apigen_generate() configured with generator 'android'")
  endif()
  ...
  set_target_properties(${apigen_java_compile_TARGET} PROPERTIES APIGEN_JAVA_COMPILE_OUTPUT_DIR
  ...
  collect_java_dependencies(${apigen_java_compile_TARGET} APIGEN_JAVA_LOCAL_DEPENDENCIES
  ...
  string(REPLACE ";" "$<SEMICOLON>" APIGEN_JAVA_LOCAL_DEPENDENCIES
                 "${APIGEN_JAVA_LOCAL_DEPENDENCIES}")
  string(REPLACE ";" "$<SEMICOLON>" APIGEN_JAVA_LOCAL_DEPENDENCIES_DIRS
                 "${APIGEN_JAVA_LOCAL_DEPENDENCIES_DIRS}")
  string(REPLACE ";" "$<SEMICOLON>" APIGEN_JAVA_REMOTE_DEPENDENCIES
                 "${APIGEN_JAVA_REMOTE_DEPENDENCIES}")
  string(REPLACE ";" "$<SEMICOLON>" APIGEN_JAVA_LOCAL_JARS "${APIGEN_JAVA_LOCAL_JARS}")
  string(REPLACE ";" "$<SEMICOLON>" APIGEN_JAVA_LOCAL_SOURCES_DIRS
                 "${APIGEN_JAVA_LOCAL_SOURCES_DIRS}")

  add_custom_command(
    TARGET ${apigen_java_compile_TARGET}
    POST_BUILD
    COMMAND
      ${CMAKE_COMMAND} -DAPIGEN_JAVA_COMPILE_OUTPUT_DIR=${APIGEN_JAVA_COMPILE_OUTPUT_DIR}
      ...
      -DGLUECODIUM_DETAILS_DIR=${GLUECODIUM_DETAILS_DIR} -P ${APIGEN_COMPILE_DIR}/runCompile.cmake
