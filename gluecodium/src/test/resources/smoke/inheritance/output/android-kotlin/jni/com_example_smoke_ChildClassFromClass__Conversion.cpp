...
REGISTER_JNI_CLASS_CACHE_INHERITANCE("com/example/smoke/ChildClassFromClass", com_example_smoke_ChildClassFromClass, "smoke_ChildClassFromClass", ::smoke::ChildClassFromClass)
...
std::shared_ptr<::smoke::ChildClassFromClass> convert_from_jni(JNIEnv* _env, const JniReference<jobject>& _jobj, TypeId<std::shared_ptr<::smoke::ChildClassFromClass>>)
{
    ...
    if (_env->IsInstanceOf(_jobj.get(), nativeBaseClass.get()))
    {
        if (_jobj != nullptr)
        {
            ...
            _nresult = *reinterpret_cast<std::shared_ptr<::smoke::ChildClassFromClass>*>(long_ptr);
        }
    }
    else
    {
