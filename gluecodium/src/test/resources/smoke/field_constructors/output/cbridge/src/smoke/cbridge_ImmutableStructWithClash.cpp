...
smoke_ImmutableStructWithClash_create_optional_handle(_baseRef stringField, int32_t intField, bool boolField)
{
    ...
    auto _intField = intField;
    auto _boolField = boolField;
    auto _struct = new ( ::std::nothrow ) std::optional<::smoke::ImmutableStructWithClash>( ::smoke::ImmutableStructWithClash( _boolField, _intField, _stringField ) );
    return reinterpret_cast<_baseRef>( _struct );
}
_baseRef
smoke_ImmutableStructWithClash_unwrap_optional_handle( _baseRef handle )
{
    return reinterpret_cast<_baseRef>( &**reinterpret_cast<std::optional<::smoke::ImmutableStructWithClash>*>( handle ) );
}
