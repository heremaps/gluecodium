...
//
import Foundation
public struct PublicTypeCollection {
...
internal func copyFromCType(_ handle: _baseRef) -> PublicTypeCollection.InternalStruct {
    return PublicTypeCollection.InternalStruct(cHandle: handle)
}
internal func moveFromCType(_ handle: _baseRef) -> PublicTypeCollection.InternalStruct {
    defer {
        smoke_PublicTypeCollection_InternalStruct_release_handle(handle)
    }
    return copyFromCType(handle)
}
internal func copyToCType(_ swiftType: PublicTypeCollection.InternalStruct) -> RefHolder {
    let c_stringField = moveToCType(swiftType.stringField)
    return RefHolder(smoke_PublicTypeCollection_InternalStruct_create_handle(c_stringField.ref))
}
internal func moveToCType(_ swiftType: PublicTypeCollection.InternalStruct) -> RefHolder {
...
internal func copyFromCType(_ handle: _baseRef) -> PublicTypeCollection.InternalStruct? {
    ...
    return PublicTypeCollection.InternalStruct(cHandle: unwrappedHandle) as PublicTypeCollection.InternalStruct
}
internal func moveFromCType(_ handle: _baseRef) -> PublicTypeCollection.InternalStruct? {
    defer {
