...
object FrancaBasedLimeModelLoader : LimeModelLoader {
    ...
    override fun loadModel(fileNames: List<String>): LimeModel {
        ...
        val limeModelBuilder = LimeModelBuilder(francaModel.deploymentModel, limeReferenceResolver)
    ...
    private fun loadFrancaModel(fileNames: List<String>): FrancaModel {
        val inputFiles = FrancaModelLoader.listFilesRecursively(fileNames.map { File(it) })

        val francaModelLoader = francaModelLoader
        val filesValidationResult = FrancaResourcesValidator.validate(
            francaModelLoader.resourceSetProvider.get(),
            inputFiles
        )
