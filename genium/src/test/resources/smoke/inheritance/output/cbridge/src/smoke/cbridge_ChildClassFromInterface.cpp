...
void* smoke_ChildClassFromInterface_get_typed(_baseRef handle) {
    const auto& real_type_id = ::genium::get_type_repository(static_cast<std::shared_ptr<::smoke::ChildClassFromInterface>::element_type*>(nullptr)).get_id(get_pointer<std::shared_ptr<::smoke::ChildClassFromInterface>>(handle)->get());
