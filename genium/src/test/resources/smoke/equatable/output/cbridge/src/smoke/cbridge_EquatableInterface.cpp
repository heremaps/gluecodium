...
void* smoke_EquatableInterface_get_typed(_baseRef handle) {
    const auto& real_type_id = ::genium::get_type_repository(static_cast<std::shared_ptr<::smoke::EquatableInterface>::element_type*>(nullptr)).get_id(get_pointer<std::shared_ptr<::smoke::EquatableInterface>>(handle)->get());
