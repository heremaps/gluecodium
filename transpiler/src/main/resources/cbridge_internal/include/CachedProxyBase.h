...
public:
    ...
    static std::shared_ptr<ProxyType> get_proxy(FunctionTable&& function_table) {
        ...
        if (cached_weak != s_cache.end()) {
            ...
            if (cached) {
                // swift pointer was retained when function table was filled, release it when no proxy is created
