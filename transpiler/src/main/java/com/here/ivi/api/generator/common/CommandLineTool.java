...
/**
 * Generic implementation of command line tool to be run on each file generated by {@link com.here.ivi.api.generator.common.GeneratorSuite}.
 * Executed command must accept input data on stdin and return processed data to stdout
 */
public class CommandLineTool implements IFileTool{

    static private Logger logger = Logger.getLogger(CommandLineTool.class.getName());
     ...
     * @throws IOException thrown in case of errors reading tool stdout or in case tool ended with error
     */
     ...
     * Method to actually execute command.
     * @param input content to be processed
     * @param additionalArgs additional argument to be passed to command. Can be used when ultimate command line
     *                       depends on content to be processed
     * @return processed data
     * @throws IOException thrown in case of errors reading tool stdout or in case tool ended with error
     */
    protected CharSequence executeCommand(CharSequence input, List<String> additionalArgs) throws Exception {
        ...
        try {
            ...
            if (input.length() > 0) {
                try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(stdin))) {
                    ...
                    logger.severe(String.format("Writing to stdin of tool '%s' failed with: %s", cmd, e.toString()));
                }
            }
            ...
            try (BufferedReader errorReader = new BufferedReader(new InputStreamReader(stderr))) {
                ...
                logger.severe(String.format("Reading stderr of tool '%s' failed with error: %s", cmd, e.toString()));
            }
            ...
            if (processExitValue != 0 || !error.isEmpty()) {
                ...
                errorMessage.appendln(String.format("Tool '%s' failed with code: %d.", cmd, processExitValue));
                if (!error.isEmpty()) {
