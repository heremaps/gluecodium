  ...
  public void helloWorldGeneration() {
    ...
    final String expected =
        "import Foundation\n"
            ...
            + "        if let result_string_handle = HelloWorld_helloWorldMethod(inputString),\n"
            + "            let ret_value = String(data: Data(bytes: HelloWorld_helloWorldMethod_getData(result_string_handle),\n"
            + "                                                count: Int(HelloWorld_helloWorldMethod_getSize(result_string_handle))), encoding: .utf8) {\n"
            + "            HelloWorld_helloWorldMethod_release(result_string_handle)\n"
            + "            return ret_value\n"
            + "        }\n"
  ...
  public void methodWithMultipleByteBufferInputStringAndIntegerReturningData() {
    ...
    SwiftMethod method =
        new SwiftMethod(
            "testBuffer", new ArrayList<>(Arrays.asList(param1, param2, param3, param4)));
    method.isStatic = true;
    method.returnType = new SwiftType("Data");
    final String expected =
        "import Foundation\n"
            + "public class HelloWorld {\n"
            ...
            + "                if let result_data_handle = HelloWorld_testBuffer(byteBuffer_ptr, Int64(byteBuffer.count), text, number, data2_ptr, Int64(data2.count)) {\n"
            + "                    let size = HelloWorld_testBuffer_getSize(result_data_handle)\n"
            + "                    if size > 0 {\n"
            + "                        let data_handle = HelloWorld_testBuffer_getData(result_data_handle)\n"
            + "                        if let data_handle = data_handle {\n"
            + "                            let return_data = Data(bytes: data_handle, count: Int(size))\n"
            + "                            HelloWorld_testBuffer_release(result_data_handle)\n"
            + "                            return return_data\n"
            + "                        }\n"
            + "                    }\n"
            + "                }\n"
